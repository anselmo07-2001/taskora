...
<div class="container custom-container">
    ...
    <div class="mb-3 d-flex justify-content-between">
        <div class="d-flex align-items-center gap-2">
            <a href="<?= BASE_URL . "/index.php?page=mySoloTasks&" . http_build_query(["filter" => "all"])?>" 
                    class="btn custom-primary-btn filter-form-btn <?= $filter === "all" ? "filter-active" : "" ?>">
                All
            </a>
            <a href="<?= BASE_URL . "/index.php?page=mySoloTasks&" . http_build_query(["filter" => "due_today"])?>" 
                    class="btn custom-primary-btn filter-form-btn <?= $filter === "due_today" ? "filter-active" : "" ?>">
                Due Today
            </a>
            <a href="<?= BASE_URL . "/index.php?page=mySoloTasks&" . http_build_query(["filter" => "overdue"])?>" 
                    class="btn custom-primary-btn filter-form-btn <?= $filter === "overdue" ? "filter-active" : "" ?>">
                Overdue
            </a>
            <a href="<?= BASE_URL . "/index.php?page=mySoloTasks&" . http_build_query(["filter" => "upcoming"])?>" 
        ...
        <form method="GET" action="<?= BASE_URL . "/index.php" ?>" class="d-flex gap-2">
                <input type="hidden" name="page" value="mySoloTasks" />
